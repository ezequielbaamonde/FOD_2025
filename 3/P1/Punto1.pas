{1. Modificar el ejercicio 4 de la práctica 1 (programa de gestión de empleados), agregándole una opción
para realizar bajas copiando el último registro del archivo en la posición del registro a borrar y luego
...
procedure bajaFisica(var arc: archivo; var numEmp: integer); //Recibe el archivo y el empleado a eliminar
 var regEmp, regUlt: empleados; pos: integer;
 begin
    reset(arc); //Apertura
    read(arc, regEmp); //Leo el primer registro
    while (not eof(arc)) and (numEmp <> regEmp.num) do begin
        read(arc, regEmp); //Leo el siguiente registro
    end;
    if (numEmp = regEmp.num) then begin
        pos:= FilePos(arc)-1; //Guardo la posicion del registro a eliminar
        seek(arc, FileSize(arc)-1); //Me paro en el ultimo registro
        read(arc, regUlt); //Leo el ultimo registro y mi puntero apunta a EOF
        seek(arc, pos); //Me posiciono en el registro a eliminar
        write(arc, regUlt); //Escribo el ultimo registro en la posicion del registro a eliminar
        seek(arc, FileSize(arc)-1); //Me posiciono una posicion antes del ultimo registro
        truncate(arc); //Trunco el archivo para eliminar el registro que reescribi
    end
    else writeln('No existe un empleado con ese numero de empleado.');
    close(arc); //Cierre
 end;
...
BEGIN
	...
	case op of
        ...
        'F': bajaFisica(registros, 1); //Ejemplo de baja fisica
	else
